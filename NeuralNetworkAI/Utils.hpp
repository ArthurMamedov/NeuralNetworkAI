...
//		return zipper<T, U>{std::forward<T>(t), std::forward<U>(u)};
	...
	/*template<class T>
	concept Productable = requires(T t) {
		{t * t} -> std::same_as<T>;
	};
	...
	template<typename T>
	using constVectorIterator = std::vector<T>::const_iterator;

	template<typename T>
	using vectorIterator = std::vector<T>::iterator;

	template<Productable T>
	class ConstVectorProductIterator {
	public:
		ConstVectorProductIterator(const constVectorIterator<T>& iter1, const constVectorIterator<T>& iter2)
			: iter1(iter1), iter2(iter2) {}

		ConstVectorProductIterator& operator++(int) {
			ConstVectorProductIterator tmp = *this;
			++(*this);
			return tmp;
		}

		ConstVectorProductIterator& operator++() {
			++iter1;
			++iter2;
			return *this;
		}

		auto operator<=>(const ConstVectorProductIterator& iter) const noexcept = default;
	...
	private:
		constVectorIterator<T> iter1;
		constVectorIterator<T> iter2;
	};

	template<Subtractable S>
	class ConstVectorSubtractIterator {
	public:
		ConstVectorSubtractIterator(const constVectorIterator<S>& iter1, const constVectorIterator<S>& iter2)
			: iter1(iter1), iter2(iter2) {}

		ConstVectorSubtractIterator& operator++(int) {
			ConstVectorSubtractIterator tmp = *this;
			++(*this);
			return tmp;
		}

		ConstVectorSubtractIterator& operator++() {
			++iter1;
			++iter2;
			return *this;
		}

		auto operator<=>(const ConstVectorSubtractIterator& iter) const noexcept = default;
	...
	private:
		constVectorIterator<S> iter1;
		constVectorIterator<S> iter2;
	};
	...
	public:
		struct Iterator {
			vectorIterator<T1> iter1;
			vectorIterator<T2> iter2;
		};

	public:
		DoubleVectorIterator(vectorIterator<T1>& iter1, vectorIterator<T2>& iter2)
			: iter1(iter1), iter2(iter2) {}

		DoubleVectorIterator& operator++(int) {
			DoubleVectorIterator tmp = *this;
			++(*this);
			return tmp;
		}
		...
		auto operator<=>(const DoubleVectorIterator& iter) const noexcept = default;
	...
	private:
		vectorIterator<T> iter1;
		vectorIterator<T> iter2;
	};*/
}